...
In dialogs, **Enter** or **Space** confirms the default action and **Esc** cancels; arrow keys move the focus between buttons, and Enter/Space then activate the focused one. Keyboard navigation is active only inside dialogs and menus, so arrow keys and Space never grab buttons in the main editor.
